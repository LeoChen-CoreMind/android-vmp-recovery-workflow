...
10. After independent DEX validation, execute `apk-unpack-repack` for the real 360 profile. Read the repack stage and multi-version adapter prompts; create a current-revision adapter from current-case evidence instead of reusing another version's facts.
