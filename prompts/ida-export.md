...
Populate the current request with case-specific `root_rva` and analysis evidence, run `scripts/ida/ida_dispatch_map.py`, then use `scripts/ida/build_mcp_response.py` to produce `ida/responses/rev-*/response.json`. Keep the IDA database `image_base` separate from `analysis.pointer_base`: a runtime dump may be loaded at image base zero while relocated dispatch slots still contain absolute pointers based on metadata `loadStart`. The response must contain `status`, exact `binary_sha256`, 256 entries, one width candidate per opcode, `width_candidates`, evidence paths, `tool=ida-mcp`, and tool version.
